...
Перебор всех дочерних узлов элемента:
...
С помощью *метода* `document.querySelector("div.article")` выбираем элемент `div` с классом `article` и пробегаемся по его дочерним узлам. И в цикле выводим имя узла и его тип с помощью свойств `nodeName` и `nodeType`.

Но несмотря на то, что в *блоке* `div` на странице только три узла: `h3` и 2 параграфа, но консоль отобразит нам `семь узлов`.

image::childnodes.png[Дочерние узлы]

Дело в том, что пробелы между узлами также считаются за отдельные текстовые узлы.

Используя *свойства* `nextSibling` и `previousSibling` можно также пройтись по узлам в прямом или обратном порядке. Например, для прохода по элементам на той же станице можно использовать следующий код *javascript*:
...
Тут все аналогично, только в цикле мы обращаемся к *следующему узлу*, который находится на *одном уровне* с текущим.
...
Если необходимо вставить новый узел на второе, третье или любое другое место, то нам надо найти узел, перед которым надо вставлять, с помощью комбинаций свойств `firstChild/lastChild` и `nextSibling/previousSibling`.

...
Объект `location` содержит информацию о расположении текущей web-страницы: _URL_, _информацию о сервере_, _номер порта_, _протокол_. С помощью свойств объекта мы можем получить эту информацию:

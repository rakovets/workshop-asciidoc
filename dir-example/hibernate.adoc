...
----
        ...
        Язык запросов Hibernate (HQL) поддерживает различные методы аггрегации, которые доступны и в SQL. HQL поддреживает слудующие методы:

min(имя свойства):: Минимальное значение данного свойства.

max(имя свойства):: Максимальное значение данного свойтсва.

sum(имя свойства):: Сумма всех значений данного свойтсва.
